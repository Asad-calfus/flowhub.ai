...
- **Phase 2+ (categorization, sentiment, few-shot classification)**: only the *input* fields
  documented in `docs/data_dictionary.md` should be passed to a classifier. The label fields
  ...
  fed in as input — see `docs/dataset_plan.md` for the data-leakage rule.
...
Run `python3 scripts/validate_dataset.py` from the repository root to check dataset
integrity (unique IDs, valid label values, valid context references, correct record counts,
etc.). See `docs/dataset_summary.md` for current statistics.
